...
do id = 1 to 100 ;
    ...
    censor = 0 ;
    do time = 0 to 20 ;
	  if eventb = 0 and censor = 0 then do ;
	      eligible= rand('bernoulli',0.7);
	      treatment = rand('bernoulli',0.5);
	      eventb = rand('bernoulli',0.01);
          censor = rand('bernoulli',0.02);
          a = rand('normal') ;
          b = rand('uniform') ;
          c = rand('bernoulli',0.2);
          z = rand('uniform');
          if eventb = 1 and censor = 1 then censor = 0 ;
	      if time = 20 then eventb = . ;
          if time = 20 then censor = 0 ;
          output ;
	  end;
    end;
end;
...
data test;
set test ;
*if time = 17 and not(id in (1,2,5,6)) then eligible = 0 ;
*if time = 17 and not(id in (1,2,43,59,65)) then eligible = 0 ; * remove most possible controls ;
*if time = 2 and id in (10,11,21,32,39,48,62,65,75,86,87,93) then eligible = 0; *  remove cases for m = 2 ;
call streaminit(1);
period = rand('bernoulli',0.8);
if z > 0.8 then period = 1 ;
else period = 0 ;
run;
...
proc freq data = test ; tables eventb censor; run;
%ccc(      datain = test ,


           checked_data = mychecks ,
		   check_for_cases =0 ,
		   checked_for_cases = 0,
		   recheck_for_cases = 0,
           additional_controls = 5 ,
		   ...
		   treatment =  treatment,
           eligible= eligible ,
          /* eligible_controls = period ,*/
          /* eligible_controls = (z > 0.8) ,*/
		   
		   ccratio= 5 ,
		   pool_models=1, 
           outcomeCov = a b c ,
		   outcomeCov_var = a b c ,
		   eligible_wts_0 = ,
		   eligible_wts_1= ,
		   cov_switchd = time a b c ,
		   class_switchd = ,
		   cov_switchn =   ,
		   class_switchn = ,
		   cense =  censor  ,
		   cov_censed = time treatment a b c ,
		   class_censed= ,
		   cov_censen = a ,
		   class_censen = ,
		   calculate_var = 1,
